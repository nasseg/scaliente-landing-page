...
import { X, Check, FileSpreadsheet, AlertTriangle, Zap, ArrowRight, MousePointer2, TrendingUp, DollarSign, Clock, RotateCcw, LayoutDashboard } from 'lucide-react';
...
const BrandIcons = {
    ...
    Meta: () => (
        <svg viewBox="0 0 24 24" className="w-4 h-4" fill="#0668E1">
    ...
    Shopify: () => (
        <svg viewBox="0 0 24 24" className="w-4 h-4" fill="#96bf48">
...
const BrowserWindow = ({ children, url, tabs = [], activeTabIndex = 0, className = "", compact = false }) => (
    <div className={`bg-[#0c0c0e] rounded-2xl border border-white/[0.08] overflow-hidden shadow-2xl flex flex-col ${className}`}>
        {/* Window Chrome */}
        <div className="bg-[#09090b] px-4 pt-3 pb-0 flex items-end gap-4 border-b border-white/[0.05]">
            ...
            {/* Tabs */}
            <div className="flex-1 flex items-end gap-1 overflow-hidden mask-linear-fade">
                {tabs.map((tab, idx) => (
                    <div
                        key={idx}
                        className={`
                            ...
                            ${idx === activeTabIndex
                                ? 'bg-[#0c0c0e] text-zinc-200 border-t border-x border-white/[0.05] relative z-10'
                                : 'bg-white/[0.02] text-zinc-600 hover:bg-white/[0.04] hover:text-zinc-400 mb-0.5'}
                        `}
                        title={tab.name}
                    >
        ...
        {/* Address Bar */}
        <div className="bg-[#0c0c0e] p-2 border-b border-white/[0.05] flex items-center gap-3">
            <div className="flex gap-2 text-zinc-600">
                <div className="w-4 h-4 rounded-full border border-zinc-800" />
                <div className="w-4 h-4 rounded-full border border-zinc-800" />
                <div className="w-4 h-4 rounded-full border border-zinc-800" />
            </div>
            <div className="flex-1 bg-[#09090b] rounded-lg h-7 flex items-center px-3 text-[10px] text-zinc-500 font-mono overflow-hidden border border-white/[0.03]">
                <span className="opacity-50 mr-1">https://</span>{url}
            </div>
        </div>

        {/* Viewport */}
        <div className="flex-1 relative bg-[#111113] overflow-hidden">
            {children}
        </div>
    </div>
);
                    ...
                    >
                        ...
                        <BrowserWindow
                            ...
                            tabs={[
                                ...
                                { name: 'Google', icon: <BrandIcons.Google /> },
                                { name: 'TikTok', icon: <BrandIcons.TikTok /> },
                                { name: 'Shopify', icon: <BrandIcons.Shopify /> },
                            ]}
                        >
                            <div className="bg-white h-full w-full text-[10px] sm:text-xs text-black font-mono overflow-hidden flex flex-col">
                                <div className="bg-[#1D6F42] text-white p-2 flex items-center gap-2">
                                ...
                                <div className="bg-gray-100 border-b border-gray-300 p-1">
                                    <div className="bg-white border border-gray-300 px-2 py-0.5 rounded text-gray-500">
                                        =SUM(B2:B10) - (C2*1.2)
                                    </div>
                                </div>
                                <div className="flex-1 bg-white p-4 relative">
                                    <div className="grid grid-cols-5 gap-px bg-gray-200 border border-gray-300">
                                        ...
                                        <div className="bg-white p-1.5">01/12</div>
                                        <div className="bg-white p-1.5">1250€</div>
                                        <div className="bg-white p-1.5">?</div>
                                        <div className="bg-white p-1.5">450€</div>
                                        <div className="bg-red-50 p-1.5 text-red-500 font-bold">#REF!</div>
                                        <div className="bg-white p-1.5">02/12</div>
                                        <div className="bg-white p-1.5">1420€</div>
                                        <div className="bg-white p-1.5">500€</div>
                                        <div className="bg-white p-1.5">?</div>
                                        <div className="bg-yellow-50 p-1.5 text-yellow-600 font-bold">#DIV/0</div>
                                    </div>
                                    <div className="absolute bottom-10 right-10">
                                        <MousePointer2 className="w-6 h-6 text-black fill-white drop-shadow-xl animate-bounce" />
                                    </div>
                                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white shadow-2xl border border-gray-200 p-3 rounded-lg flex items-center gap-2 min-w-[180px]">
                                        <AlertTriangle className="w-5 h-5 text-red-500" />
                                        <span className="text-red-600 font-bold text-xs">{content?.before?.excel?.error}</span>
                                    </div>
                                </div>
                            </div>
                        </BrowserWindow>
                        ...
                        >
                            ...
                            <div className="bg-[#fafafa] h-full w-full flex">
                                ...
                                <div className="w-12 bg-white border-r border-zinc-100 py-3 hidden sm:flex flex-col items-center gap-3">
                                    ...
                                    <div className="w-7 h-7 rounded-lg hover:bg-zinc-50 flex items-center justify-center">
                                ...
                                <div className="flex-1 p-4 overflow-hidden">
                                    ...
                                    <div className="flex items-center justify-between mb-4">
                                        ...
                                        <div className="flex items-center gap-1.5 px-2 py-1 bg-green-50 rounded-full border border-green-100">
                                            ...
                                            <span className="text-green-600 text-[10px] font-semibold">{content?.after?.dashboard?.live}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 mb-2">
                                        ...
                                        <div className="col-span-2 sm:col-span-1 bg-gradient-to-br from-emerald-50 via-green-50 to-teal-50 border border-emerald-100 rounded-lg p-3">
                                            <span className="inline-block px-1.5 py-0.5 bg-emerald-500 text-white text-[8px] font-bold rounded uppercase mb-1">
                                                {content?.after?.dashboard?.netProfit || 'Net Profit'}
                                            </span>
                                            <div className="flex items-end gap-1.5">
                                                <span className="text-xl font-sans font-bold text-zinc-900">2,845€</span>
                                                <div className="w-5 h-5 bg-emerald-100 rounded flex items-center justify-center mb-0.5">
                                                    <TrendingUp className="w-3 h-3 text-emerald-600" />
                                                </div>
                                            </div>
                                            <p className="text-[9px] text-emerald-600 font-medium mt-0.5">{content?.after?.dashboard?.vsYesterday}</p>
                                        </div>

                                        {/* MARGIN */}
                                        <div className="bg-white border border-zinc-100 rounded-lg p-3">
                                            <span className="text-[8px] text-zinc-500 uppercase font-medium">{content?.after?.dashboard?.margin || 'Marge'}</span>
                                            <p className="text-xl font-sans font-bold text-zinc-900">32%</p>
                                            <p className="text-[9px] text-zinc-400">{content?.after?.dashboard?.stable}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        ...
                                        <div className="bg-white border border-zinc-100 rounded-lg p-2">
                                            <div className="flex items-center gap-1 mb-0.5">
                                                ...
                                                <span className="text-[7px] text-zinc-500 uppercase">Revenue</span>
                                            </div>
                                            <p className="text-sm font-sans font-bold text-zinc-900">8,890€</p>
                                        </div>

                                        {/* Pending - Orange */}
                                        <div className="bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-100 rounded-lg p-2">
                                            <div className="flex items-center gap-1 mb-0.5">
                                                <Clock className="w-3 h-3 text-orange-500" />
                                                <span className="text-[7px] text-orange-600 uppercase">Pending</span>
                                            </div>
                                            <p className="text-sm font-sans font-bold text-zinc-900">12</p>
                                        </div>

                                        {/* Refunded - Pink */}
                                        <div className="bg-gradient-to-br from-pink-50 to-rose-50 border border-pink-100 rounded-lg p-2">
                                            <div className="flex items-center gap-1 mb-0.5">
                                                <RotateCcw className="w-3 h-3 text-pink-500" />
                                                <span className="text-[7px] text-pink-600 uppercase">Refunded</span>
                                            </div>
                                            <p className="text-sm font-sans font-bold text-zinc-900">0€</p>
                                        </div>
                                    </div>
